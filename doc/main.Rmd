...
nprinciple = c(650, 700, 730, 740, 750, 800)
)
...
#### Principal Components Analysis (PCA) + Support Vector Machines (SVMs): Train a classification model with training features and responses
...
if(run.cv.svm){
  ...
  for(i in 1:length(hyper_grid_svm$nprinciple)){
    ...
    res_cv_svm[i,] <- cv.function(features = feature_train, labels = label_train, K,pca1,np=hyper_grid_svm$nprinciple[i], reweight = sample.reweight)
...
* Choose the "best" parameter value
The best is number of pc equal to 740
